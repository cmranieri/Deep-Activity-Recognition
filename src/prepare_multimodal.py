...
root = '/media/olorin/Documentos/caetano/multimodal_dataset'
trainfiletxt = open( '../splits/multimodal_dataset/trainlist.txt' , 'w' )
testfiletxt  = open( '../splits/multimodal_dataset/testlist.txt'  , 'w' )
trainlist = list()
testlist  = list()

classes = sorted( os.listdir( os.path.join( root , 'video' ) ) )
for c in classes:
    files = sorted( os.listdir( os.path.join( root , 'video' , c ) ) )
    for f in files[ 0 : 7 ]:
